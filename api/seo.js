...
export default async function handler(req, res) {
  // Setup standard CORS headers for development
...
- Original Description: ${description || 'Premium craftsmanship design.'}
...
Ensure the Keywords are a list of 5 to 8 comma-separated search-term keywords ideal for indexing and discoverability.`;
    ...
    console.error('Gemini SEO Generator handler failed:', err);
    res.setHeader('Content-Type', 'application/json');
    return res.status(500).json({ 
      error: 'Gemini Generation Failure', 
      details: err.message 
    });
  }
}
